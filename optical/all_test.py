...
parser.add_argument("--sup_data_name", type=str, default='optical_train_pool')
parser.add_argument("--que_data_name", type=str, default='optical_test_pool')
...
def main():
    ...
    sup_stacks_1 = torch.Tensor(np.load('./data/' + args.sup_data_name + '/stacks_1.npy'))
    sup_stacks_2 = torch.Tensor(np.load('./data/' + args.sup_data_name + '/stacks_2.npy'))
    sup_stacks_3 = torch.Tensor(np.load('./data/' + args.sup_data_name + '/stacks_3.npy'))
    ...
    que_stacks_1 = torch.Tensor(np.load('./data/' + args.que_data_name + '/stacks_1.npy'))
    que_stacks_2 = torch.Tensor(np.load('./data/' + args.que_data_name + '/stacks_2.npy'))
    que_stacks_3 = torch.Tensor(np.load('./data/' + args.que_data_name + '/stacks_3.npy'))
    ...
    for epoch in range(args.num_epoch):
        ...
        pro = np.zeros((que_stacks_1.shape[0], args.train_n_way), dtype=float)
        np.random.seed(1012)
        ...
        for sup_epoch in range(args.epoch_sup):
            ...
            for que_epoch in tqdm(range(int(epoch_que))):
                ...
                for k in range(args.train_n_way):
                    pro[index[index_select][k]] = F.softmax(relations[k])
